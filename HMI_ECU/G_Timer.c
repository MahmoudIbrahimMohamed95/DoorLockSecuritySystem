...
****************************************************************************************************************/
 volatile static void (*g_Timer0_CallBackPtr)(void)= NULL_PTR;
  volatile static  void (*g_Timer1_CallBackPtr)(void)= NULL_PTR;
 volatile static  void (*g_Timer2_CallBackPtr)(void)= NULL_PTR;
...
void Timer_stop (const TIMER_Config*Config_Ptr){
	...
	case TIMER1:
		...
		case COMPAREMODE :
			...
			OCR1AL  =   0; // Set Compare Value

			OCR1AH  = 0; // Set Compare Value
	...
	case TIMER2:
		...
		case COMPAREMODE:
			...
			TCNT2 = 0;    // Set Timer initial value to 0
			OCR2  = 0; // Set Compare Value 0
			TIMSK &=~(1<<OCIE2); // disEnable Timer2 Compare Interrupt
